...
RETURNS TABLE (
  ...
  content TEXT,
  score NUMERIC(10, 5),
  vector_similarity NUMERIC(10, 5),
  fts_rank NUMERIC(10, 5),
  recency_score NUMERIC(10, 5),
  access_score NUMERIC(10, 5),
  importance NUMERIC(10, 5),
  confidence NUMERIC(10, 5),
  contextual_boost NUMERIC(10, 5)
) AS $$
DECLARE
...
BEGIN
  ...
  WITH vector_scores AS (
    SELECT 
      ...
      m.metadata,
      m."confidenceScore",
      (1 - (e.embedding <=> p_query_embedding))::NUMERIC(10, 5) as vector_sim
    FROM memories m
  ...
  combined AS (
    SELECT 
      ...
      v."confidenceScore",
      v.vector_sim,
      COALESCE(f.fts_rank, 0)::NUMERIC(10, 5) as fts_rank,
      
      -- Recency score
      EXP(-recency_decay_rate * EXTRACT(EPOCH FROM (NOW() - v."createdAt")) / 86400.0)::NUMERIC(10, 5) as recency,
      
      -- Access frequency score
      ((LN(1 + v."accessCount") / LN(1 + max_access_count)) * 
      EXP(-recency_decay_rate * EXTRACT(EPOCH FROM (NOW() - v."lastAccessedAt")) / 86400.0))::NUMERIC(10, 5) as access_freq,
      
      -- Importance from metadata
      COALESCE((v.metadata->>'importance')::NUMERIC(10, 5), 0.5) as importance,
      ...
      CASE 
        WHEN array_length(p_context_entities, 1) > 0 THEN
          ...
          )::NUMERIC(10, 5) / array_length(p_context_entities, 1)
        ELSE 0
  ...
  SELECT 
    ...
    )::NUMERIC(10, 5) as final_score,
    c.vector_sim,
    c.fts_rank,
    c.recency,
    c.access_freq,
    c.importance,
    c."confidenceScore"::NUMERIC(10, 5) as confidence,
    c.contextual_boost
  FROM combined c
  WHERE (
    c.vector_sim * 0.35 +
